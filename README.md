...
**沉浸式 AWS Solutions Architect Associate (SAA-C03) 學習平台**

一個極簡、無干擾的開源學習工具，幫助你高效掌握 AWS SAA-C03 考試所有核心概念。
...
| 📝 **模擬測驗 (Quizzes)** | 從知識庫隨機生成 10-30 題選擇題，即時批改，模擬真實考試體驗 |
| 📊 **進度儀表板 (Dashboard)** | 環形進度圖 + GitHub 風格熱力圖，一眼掌握學習狀態 |
| 🔍 **全局搜索 (⌘K)** | 快速搜索任意 AWS 概念（支持英文 / 中文模糊匹配） |
...
│   ├── components/
│   │   ├── TopicCard.jsx       # 概念卡片（含打勾微動效）
│   │   ├── ProgressRing.jsx    # SVG 環形進度圖
│   │   └── CommandPalette.jsx  # ⌘K 全局搜索面板
│   ├── pages/
│   │   ├── Hub.jsx             # 知識庫主頁
...
- **Flashcards** — Immersive full-screen card flip review with keyboard navigation (← → to navigate, Space to flip)
- **Quiz Generator** — Randomly generated multiple-choice quizzes (10–30 questions) with instant grading
- **Progress Dashboard** — Ring chart + GitHub-style heatmap to visualize mastery status
- **Global Search (⌘K)** — Instant search across all concepts (English & Chinese)
- **Local-first** — Zero backend; all progress persists in browser localStorage with JSON export/import
- **Dark/Light Theme** — Deep midnight-blue dark theme with warm gold accents; one-click toggle
...
**Built by ElodinaHan for AWS certification learners**
